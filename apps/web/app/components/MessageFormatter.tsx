    ...
    <div className={`prose prose-invert max-w-none ${className}`}>
      <ReactMarkdown
        ...
        components={{
            ...
            if (inline) {
              return (
                <code className="bg-gray-800/60 text-orange-300 px-1.5 py-0.5 rounded text-sm font-mono">
                  {children}
                </code>
              );
            }
            ...
            return (
              <div className="relative group my-4">
                <div className="flex items-center justify-between bg-gray-800/50 px-4 py-2 rounded-t-lg border border-white/10">
          ...
          h2: ({ children }) => (
            <h2 className="text-xl font-bold text-white/95 mt-6 mb-3">{children}</h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-lg font-semibold text-white/90 mt-4 mb-2">{children}</h3>
          ),
          p: ({ children }) => (
            <p className="text-white/85 leading-relaxed my-2">{children}</p>
          ),
                     ul: ({ children }) => (
           ...
           li: ({ children, ...props }) => {
             ...
             return (
               <li className="flex items-start gap-2 text-white/85 leading-relaxed">
                 <span className="text-white/60 mt-0.5 text-sm min-w-fit">
                   {isOrdered ? '•' : '•'}
                 </span>
